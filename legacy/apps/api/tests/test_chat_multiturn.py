...
async def test_L_memory_injected_into_provider_request(fresh_app) -> None:
    ...
    assert "Sos Vane" in msgs[0].content
    # Second system message is a server-authored wrapper around untrusted memory data.
    assert msgs[1].content.startswith("Contexto de memoria del usuario.")
    assert "DATOS NO CONFIABLES" in msgs[1].content
...
async def test_M_no_memories_chat_unaffected(fresh_app) -> None:
    ...
    for m in msgs:
        assert "Contexto de memoria del usuario" not in m.content
